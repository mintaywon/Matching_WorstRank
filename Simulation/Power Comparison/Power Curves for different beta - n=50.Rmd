...
simulate.exp.both <- function(n, q2, HR, Delta, time, rho_Y, rho_t){
  ...
  if (OnlyControlDead == 0 & OnlyTreatedDead == 0) return (c(-1,-1,-1)); # no discordant pair
  # T1 : binomial
  pval = binom.test(c(OnlyControlDead, OnlyTreatedDead), alternative = "greater")$p.value;
  if (pval <= alpha) {return (c(pval, pval.2, pval.3));}
  else { 
    ...
    if (pval.alt > alpha) { return (c(pval, pval.2, pval.3)); }
    else {
      return (c(1 - rbinom(1, 1, (alpha - pval.alt)/(pval - pval.alt)), pval.2, pval.3)); # auxiliary random 
    }
  }
}
```
...
```{r}
power.exp.seqT1T2 = array(data = NA, dim=c(length(q2), length(Delta), length(HR), length(beta)), dimnames = list(c("q2=60%", "q2=80%", "q2=90%"), c(0.0, 0.1, 0.2, 0.3), c(1.0, 1.2, 1.4, 1.6), seq(0, 0.05, by=5e-4)))

power.exp.seqT1T3 = array(data = NA, dim=c(length(q2), length(Delta), length(HR), length(beta)), dimnames = list(c("q2=60%", "q2=80%", "q2=90%"), c(0.0, 0.1, 0.2, 0.3), c(1.0, 1.2, 1.4, 1.6), seq(0, 0.05, by=5e-4)))
      ...
      for (l in c(1:N)) {
        ...
        for (b in 1:length(beta)) {
          if (pvals[1] <= beta[b]) {
            ...
            if (pvals[2] <= (alpha - beta[b])) {count12[b] = count12[b] + 1}
            if (pvals[3] <= (alpha - beta[b])) {count13[b] = count13[b] + 1}
          }
        }
      }
